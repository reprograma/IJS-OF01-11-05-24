...
test('deve retornar pacote_intermedio quando a duração do projeto é de 51 horas', () => {
    const esperado = 'pacote_intermediario';
    const retornado = calcularPacote(51);
    expect(esperado).toBe(retornado);
});

test('deve retornar pacote_premium quando a duração do projeto é de 176 horas', () => {
    const esperado = 'pacote_premium';
    const retornado = calcularPacote(176);
    expect(esperado).toBe(retornado);
});

test('deve retornar pacote_basico quando a duração do projeto é de 17 horas', () => {
    const esperado = 'pacote_basico';
    const retornado = calcularPacote(17);
    expect(esperado).toBe(retornado);
});